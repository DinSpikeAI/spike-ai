...
export default function AuthPage() {
  ...
  return (
    <div className="min-h-screen bg-[#060608] flex flex-col relative overflow-hidden">
      ...
      {/* ═══ Ambient Background — stronger glow ═══ */}
      <div className="fixed inset-0 pointer-events-none">
        <div className="absolute top-[25%] left-[50%] -translate-x-1/2 w-[900px] h-[600px] rounded-full opacity-[0.07]"
          style={{ background: "radial-gradient(ellipse, rgba(99,102,241,0.8) 0%, rgba(139,92,246,0.4) 30%, transparent 70%)", animation: "glow 12s ease-in-out infinite" }} />
        <div className="absolute bottom-[10%] left-[40%] w-[500px] h-[300px] rounded-full opacity-[0.03]"
      ...
      <div className="flex-1 flex items-center justify-center px-6 pb-20 relative z-20">
        <div className={`w-full max-w-[440px] text-center transition-all duration-500 ${transitioning ? "opacity-0 scale-[0.96]" : "opacity-100 scale-100"}`}>
          ...
          {view === "magic-sent" && (
            <div style={{ animation: "reveal 0.7s cubic-bezier(0.16,1,0.3,1)" }}>
              <div className="relative inline-block mb-12">
                <div className="w-28 h-28 rounded-[28px] bg-gradient-to-br from-blue-500/20 to-indigo-500/10 border border-blue-400/10 flex items-center justify-center"><Mail size={40} className="text-blue-300/50" /></div>
                <div className="absolute -inset-6 rounded-[40px] bg-blue-500/[0.04] blur-2xl -z-10" />
              </div>
              <h1 className="text-[44px] md:text-[52px] font-bold tracking-[-0.02em] leading-[1.08] mb-5">Check your inbox</h1>
          ...
          {view === "signup-done" && (
            <div style={{ animation: "reveal 0.7s cubic-bezier(0.16,1,0.3,1)" }}>
              <div className="relative inline-block mb-12">
                <div className="w-28 h-28 rounded-[28px] bg-gradient-to-br from-emerald-500/20 to-green-500/10 border border-emerald-400/10 flex items-center justify-center"><CheckCircle size={40} className="text-emerald-300/50" /></div>
                <div className="absolute -inset-6 rounded-[40px] bg-emerald-500/[0.04] blur-2xl -z-10" />
              </div>
              <h1 className="text-[44px] md:text-[52px] font-bold tracking-[-0.02em] leading-[1.08] mb-5">You&apos;re in</h1>
          ...
          {view === "setup-profile" && (
            <div style={{ animation: "reveal 0.7s cubic-bezier(0.16,1,0.3,1)" }} className="max-w-[520px] mx-auto">
              <h1 className="text-[44px] md:text-[52px] font-bold tracking-[-0.02em] leading-[1.08] mb-3">Who&apos;s watching?</h1>
              ...
              <div className="max-w-[340px] mx-auto mb-12">
                <input type="text" value={displayName} onChange={(e) => setDisplayName(e.target.value)} placeholder="Your name"
                  className="w-full px-6 py-[17px] bg-white/[0.04] border border-white/[0.08] rounded-2xl text-[17px] text-white placeholder-white/15 focus:outline-none focus:border-indigo-500/40 focus:bg-white/[0.06] focus:shadow-[0_0_30px_rgba(99,102,241,0.08)] transition-all text-center tracking-wide" />
              </div>
          ...
          {(view === "signin" || view === "signup" || view === "magic") && (
            <div style={{ animation: "reveal 0.7s cubic-bezier(0.16,1,0.3,1)" }}>
              {/* Title — big, tight tracking */}
              <h1 className="text-[50px] md:text-[62px] font-bold tracking-[-0.03em] leading-[1.06] mb-4">
                {view === "signin" ? "Welcome\nback." : view === "signup" ? "Create\naccount." : "Magic\nlink."}
              </h1>
              <p className="text-[17px] text-white/25 mb-16">
                {view === "signin" ? "Sign in to continue to spike AI." : view === "signup" ? "Join the future of AI cinema." : "We'll email you a sign-in link."}
              </p>

              {/* ═══ Google Sign-In ═══ */}
              {view !== "magic" && (
                <div className="max-w-[380px] mx-auto mb-8">
                  <button onClick={handleGoogleSignIn} disabled={googleLoading}
                    className="w-full flex items-center justify-center gap-3 px-6 py-[16px] bg-white/[0.04] border border-white/[0.08] hover:border-white/[0.15] hover:bg-white/[0.06] rounded-2xl text-[15px] font-medium text-white/70 transition-all cursor-pointer disabled:opacity-40">
                    {googleLoading ? <Loader2 size={18} className="animate-spin text-white/30" /> : (
                      <>
                  ...
                  <div className="flex items-center gap-4 my-8">
                    ...
                    <span className="text-[12px] text-white/15 font-medium tracking-wider">OR</span>
                    <div className="flex-1 h-px bg-white/[0.06]" />
                  </div>
                </div>
              )}
              ...
              <div className="max-w-[380px] mx-auto">
                <div className="space-y-5 text-left">
                  <div>
                    <label className="block text-[12px] font-semibold tracking-[0.2em] text-white/30 uppercase mb-3 ml-1">Email</label>
                    <input type="email" value={email} onChange={(e) => { setEmail(e.target.value); setError(null); }} onKeyDown={(e) => { if (e.key === "Enter") handleAuth(); }}
                      placeholder="you@email.com"
                      className="w-full px-6 py-[17px] bg-white/[0.03] border border-white/[0.08] rounded-2xl text-[17px] text-white placeholder-white/15 focus:outline-none focus:border-indigo-500/40 focus:bg-white/[0.05] focus:shadow-[0_0_30px_rgba(99,102,241,0.08)] transition-all tracking-wide" />
                  </div>
                  {view !== "magic" && (
                    <div style={{ animation: "slideIn 0.3s ease" }}>
                      <label className="block text-[12px] font-semibold tracking-[0.2em] text-white/30 uppercase mb-3 ml-1">{view === "signup" ? "Create Password" : "Password"}</label>
                      <div className="relative">
                        <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => { setPassword(e.target.value); setError(null); }} onKeyDown={(e) => { if (e.key === "Enter") handleAuth(); }}
                          placeholder="••••••••"
                          className="w-full px-6 py-[17px] pr-14 bg-white/[0.03] border border-white/[0.08] rounded-2xl text-[17px] text-white placeholder-white/15 focus:outline-none focus:border-indigo-500/40 focus:bg-white/[0.05] focus:shadow-[0_0_30px_rgba(99,102,241,0.08)] transition-all" />
                        <button onClick={() => setShowPassword(!showPassword)} className="absolute right-5 top-1/2 -translate-y-1/2 text-white/12 hover:text-white/30 transition-colors cursor-pointer" tabIndex={-1}>
                          {showPassword ? <EyeOff size={18} /> : <Eye size={18} />}
                        </button>
                      </div>
                      {view === "signup" && <p className="text-[11px] text-white/12 mt-2.5 ml-1">Minimum 6 characters</p>}
                    </div>
                  )}
                </div>
                ...
                {/* Error */}
                {error && (
                  <div className="flex items-start gap-3 mt-6 p-5 bg-red-500/[0.04] border border-red-500/[0.08] rounded-2xl text-left" style={{ animation: "shake 0.4s ease" }}>
                    <AlertCircle size={16} className="text-red-400/50 flex-shrink-0 mt-0.5" />
                    <p className="text-[14px] text-red-300/50 leading-relaxed">{error}</p>
                  </div>
                )}

                {/* Primary CTA — bigger, bolder, glowy */}
                <button onClick={handleAuth} disabled={loading}
                  className="cta-btn w-full mt-10 py-[19px] text-black text-[16px] font-bold tracking-wide rounded-full flex items-center justify-center gap-3 disabled:opacity-30 cursor-pointer active:scale-[0.97]">
                  {loading ? <Loader2 size={20} className="animate-spin text-black/40" />
                    : view === "magic" ? <>Send Link <Wand2 size={17} /></>
                    : view === "signup" ? <>Create Account <Sparkles size={17} /></>
                    : <>Sign In <ArrowRight size={18} strokeWidth={2.5} /></>}
                </button>

                {/* Magic link — secondary */}
                <button onClick={() => switchView(view === "magic" ? "signin" : "magic")}
                  className="w-full mt-4 py-[15px] text-[14px] text-white/20 hover:text-white/40 font-medium rounded-full border border-white/[0.06] hover:border-white/[0.12] hover:bg-white/[0.02] transition-all flex items-center justify-center gap-2.5 cursor-pointer">
                  {view === "magic" ? <><Lock size={14} /> Use password</> : <><Wand2 size={14} /> Magic link</>}
                </button>

                {/* Separator */}
                <div className="my-10"><div className="h-px bg-white/[0.04]" /></div>

                {/* Toggle — clearer */}
                <p className="text-center text-[15px] text-white/20">
                  {view === "signup" ? (
      ...
      <style jsx>{`
        ...
        @keyframes glow { 0%,100% { opacity:0.07; transform:translate(-50%,0) scale(1) } 50% { opacity:0.1; transform:translate(-50%,0) scale(1.12) } }
      `}</style>
    </div>
  );
}
