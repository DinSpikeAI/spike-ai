...
const ALL_MOVIES: Movie[] = [
  { id: "feat-1", title: "GENESIS PROTOCOL", year: 2026, rating: 9.2, duration: "2h 14m", poster: "https://picsum.photos/seed/genesis-poster/400/600", image: "https://picsum.photos/seed/genesis-wide/1920/1080", aiModels: ["Sora", "Runway Gen-4", "Stable Diffusion XL"], genre: "Sci-Fi", description: "In a world where AI has surpassed human intelligence, a rogue neural network begins rewriting reality itself.", tagline: "When the code becomes conscious, humanity faces its final test.", maturity: "16+", creator: "cr1" },
  { id: "t1", title: "The Last Render", year: 2026, rating: 8.7, duration: "1h 52m", poster: "https://picsum.photos/seed/lastrender2/400/600", aiModels: ["Sora", "ElevenLabs"], genre: "Sci-Fi", description: "A dying artist uploads his consciousness into an AI renderer, but the machine has its own vision.", creator: "cr1" },
  ...
  { id: "t3", title: "Pixel Requiem", year: 2026, rating: 9.0, duration: "1h 47m", poster: "https://picsum.photos/seed/pixelreq4/400/600", aiModels: ["Sora", "Midjourney"], genre: "Drama", description: "A haunting meditation on digital mortality as an AI grieves the deletion of its training data.", creator: "cr2" },
  { id: "t5", title: "Void Walker", year: 2026, rating: 8.5, duration: "2h 10m", poster: "https://picsum.photos/seed/voidwalk6/400/600", aiModels: ["Sora"], genre: "Sci-Fi", description: "An astronaut stranded between dimensions must navigate impossible geometries to find her way home.", creator: "cr3" },
  { id: "s1", title: "Parallax", year: 2026, rating: 9.1, duration: "2h 20m", poster: "https://picsum.photos/seed/parallax10/400/600", aiModels: ["Sora"], genre: "Sci-Fi", description: "Reality fractures into parallel timelines, each rendered in a different visual style." },
  ...
  { id: "h6", title: "Latent Space", year: 2025, rating: 8.8, duration: "1h 59m", poster: "https://picsum.photos/seed/latentsp23/400/600", aiModels: ["Sora"], genre: "Horror", description: "A researcher maps the latent space and discovers something hiding between the dimensions." },
  ...
  { id: "a1", title: "Epoch", year: 2025, rating: 9.4, duration: "2h 35m", poster: "https://picsum.photos/seed/epoch34/400/600", aiModels: ["Sora", "ElevenLabs"], genre: "Drama", description: "Spanning millennia of human civilization in 155 minutes. The most awarded AI film in history." },
  ...
  { id: "an1", title: "Neon Ronin", year: 2026, rating: 9.1, duration: "1h 55m", poster: "https://picsum.photos/seed/neonronin50/400/600", aiModels: ["Sora", "Midjourney"], genre: "Anime", description: "A masterless samurai in cyberpunk Osaka fights through neon-lit streets controlled by warring AI clans.", creator: "cr1" },
  { id: "an3", title: "Mecha Genesis", year: 2025, rating: 9.0, duration: "2h 05m", poster: "https://picsum.photos/seed/mechagen52/400/600", aiModels: ["Runway Gen-4", "Sora"], genre: "Anime", description: "Giant AI-controlled mechas defend Earth from interdimensional kaiju.", creator: "cr3" },
  { id: "an7", title: "Celestial Engine", year: 2026, rating: 9.2, duration: "2h 15m", poster: "https://picsum.photos/seed/celesteng56/400/600", aiModels: ["Sora", "ElevenLabs"], genre: "Anime", description: "A space opera spanning twelve galaxies, where an AI empress must choose between digital and organic life.", creator: "cr3" },
];
...
export default function MoviePage() {
  ...
  const similarMovies = movie ? ALL_MOVIES.filter(m => m.id !== movie.id && m.genre === movie.genre).slice(0, 10) : [];
  const idx = ALL_MOVIES.findIndex(m => m.id === movieId);
  const upNextMovie = idx >= 0 && idx < ALL_MOVIES.length - 1 ? ALL_MOVIES[idx + 1] : ALL_MOVIES[0];
  ...
  if (loading) return (<div className="min-h-screen bg-[#050505] flex items-center justify-center"><div className="flex flex-col items-center gap-4"><div className="w-10 h-10 border-2 border-[#E50914] border-t-transparent rounded-full animate-spin" /><p className="text-white/30 text-sm tracking-wider">Loading film...</p></div></div>);
  if (!movie) return (<div className="min-h-screen bg-[#050505] flex items-center justify-center"><div className="text-center"><h1 className="text-4xl font-bold text-white mb-4">Film Not Found</h1><p className="text-gray-400 mb-8">This film doesn&apos;t exist in our universe.</p><button onClick={() => router.push("/")} className="px-6 py-3 bg-[#E50914] text-white rounded-lg">Back to Home</button></div></div>);
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white">
      ...
      <section className="relative w-full aspect-video max-h-[75vh] overflow-hidden bg-black">
        {playing && embedUrl ? (
          ...
          <div className="relative w-full h-full cursor-pointer group" onClick={() => movie.video_url && setPlaying(true)}>
            ...
            {movie.video_url && (<div className="absolute inset-0 flex items-center justify-center"><div className="w-20 h-20 rounded-full bg-[#E50914]/90 flex items-center justify-center shadow-2xl shadow-[#E50914]/40 group-hover:scale-110 transition-transform duration-300"><Play size={32} fill="white" className="text-white ml-1" /></div></div>)}
          </div>
        )}
      </section>
          ...
          <div className="flex flex-col md:flex-row gap-6 md:gap-10">
            ...
            <div className="flex-1 min-w-0 pt-4 md:pt-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-2xl">
                {creator && (<div className="bg-white/[0.03] rounded-xl p-5 border border-white/5 hover:border-white/10 transition-colors"><div className="flex items-center gap-3 mb-3"><img src={creator.avatar} alt={creator.name} className="w-10 h-10 rounded-full object-cover border border-white/10" /><div><p className="text-sm font-semibold text-white">{creator.name}</p><p className="text-[10px] text-[#E50914] font-medium tracking-wider uppercase">Verified Creator</p></div></div><div className="flex items-center gap-4 mb-3"><span className="text-xs text-gray-500"><span className="text-white font-semibold">{creator.followers.toLocaleString()}</span> followers</span><span className="text-xs text-gray-500"><span className="text-white font-semibold">{creator.films}</span> films</span></div><button onClick={() => router.push(`/creator/${creator.id}`)} className="flex items-center gap-1 text-[#E50914] text-xs font-medium hover:text-[#f6121d] transition-colors">View Profile <ExternalLink size={11} /></button></div>)}
                <div className="bg-white/[0.03] rounded-xl p-5 border border-white/5 hover:border-white/10 transition-colors"><div className="flex items-center gap-2 mb-3"><Cpu size={16} className="text-[#E50914]" /><p className="text-xs text-gray-500 uppercase tracking-wider font-medium">AI Models Used</p></div><div className="flex flex-wrap gap-2">{movie.aiModels.map((m, i) => (<span key={i} className="px-3 py-1.5 text-xs font-medium bg-white/5 text-gray-300 rounded-lg border border-white/10">{m}</span>))}</div></div>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* UP NEXT */}
      {upNextMovie && (<section className="py-6 md:py-8 px-4 md:px-6"><div className="max-w-[1400px] mx-auto"><h2 className="text-lg font-bold mb-4 flex items-center gap-2"><SkipForward size={18} className="text-[#E50914]" />Up Next</h2><div onClick={() => router.push(`/movie/${upNextMovie.id}`)} className="flex items-center gap-4 md:gap-6 bg-white/[0.03] border border-white/5 hover:border-[#E50914]/20 rounded-xl p-3 md:p-4 cursor-pointer transition-all group"><div className="relative w-[70px] md:w-[90px] flex-shrink-0 aspect-[2/3] rounded-lg overflow-hidden"><img src={upNextMovie.poster} alt={upNextMovie.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" /><div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-all flex items-center justify-center"><div className="w-8 h-8 rounded-full bg-[#E50914] flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"><Play size={14} fill="white" className="text-white ml-0.5" /></div></div></div><div className="flex-1 min-w-0"><p className="text-xs text-gray-500 uppercase tracking-wider font-medium mb-1">Playing Next</p><h3 className="text-base md:text-lg font-bold text-white truncate group-hover:text-[#E50914] transition-colors">{upNextMovie.title}</h3><div className="flex items-center gap-2 mt-1"><span className="text-yellow-400 text-xs flex items-center gap-0.5"><Star size={10} fill="currentColor" />{upNextMovie.rating}</span><span className="text-gray-500 text-xs">{upNextMovie.year} · {upNextMovie.genre} · {upNextMovie.duration}</span></div></div><div className="flex-shrink-0 hidden sm:flex"><button className="flex items-center gap-2 px-5 py-2.5 bg-[#E50914] text-white rounded-lg font-semibold text-sm hover:bg-[#f6121d] transition-all"><Play size={14} fill="white" />Play Next</button></div></div></div></section>)}

      {/* MORE LIKE THIS */}
      {similarMovies.length > 0 && (<section className="py-8 md:py-12 px-4 md:px-6"><div className="max-w-[1400px] mx-auto"><h2 className="text-xl font-bold mb-6 flex items-center gap-2"><Sparkles size={18} className="text-[#E50914]" />More Like This</h2><div className="relative group"><button onClick={() => scrollSimilar("left")} className="absolute -left-4 top-1/2 -translate-y-1/2 z-10 w-10 h-10 rounded-full bg-black/80 border border-white/10 flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition-opacity"><ChevronLeft size={20} /></button><button onClick={() => scrollSimilar("right")} className="absolute -right-4 top-1/2 -translate-y-1/2 z-10 w-10 h-10 rounded-full bg-black/80 border border-white/10 flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition-opacity"><ChevronRight size={20} /></button><div ref={scrollRef} className="flex gap-4 overflow-x-auto pb-4" style={{ scrollbarWidth: "none" }}>{similarMovies.map(m => (<div key={m.id} onClick={() => router.push(`/movie/${m.id}`)} className="flex-shrink-0 w-[130px] md:w-[180px] cursor-pointer group/card"><div className="relative aspect-[2/3] rounded-lg overflow-hidden mb-2 border border-white/5 group-hover/card:border-[#E50914]/30 transition-all duration-300"><img src={m.poster} alt={m.title} className="w-full h-full object-cover transition-transform duration-500 group-hover/card:scale-110" /><div className="absolute inset-0 bg-black/0 group-hover/card:bg-black/50 transition-all flex items-center justify-center"><div className="w-12 h-12 rounded-full bg-[#E50914] flex items-center justify-center opacity-0 group-hover/card:opacity-100 scale-75 group-hover/card:scale-100 transition-all shadow-lg shadow-[#E50914]/40"><Play size={20} fill="white" className="text-white ml-0.5" /></div></div></div><p className="text-sm font-medium text-gray-200 group-hover/card:text-white transition-colors truncate">{m.title}</p><div className="flex items-center gap-2 mt-1"><span className="text-yellow-400 text-xs flex items-center gap-0.5"><Star size={10} fill="currentColor" />{m.rating}</span><span className="text-gray-500 text-xs">{m.year}</span></div></div>))}</div></div></div></section>)}

      {/* FOOTER */}
      <footer className="py-8 md:py-12 px-4 md:px-6 border-t border-white/5"><div className="max-w-[1400px] mx-auto text-center"><div className="flex items-center justify-center gap-2 mb-2"><img src="/mascot.png" alt="Spike" className="h-8 w-auto" /><span className="text-lg font-black tracking-tight text-white">spike</span><span className="text-lg font-black tracking-tight text-[#E50914]" style={{ marginLeft: "-4px" }}>AI</span></div><p className="text-[10px] tracking-[0.3em] uppercase text-gray-600 mb-4">AI Cinema</p><p className="text-gray-500 text-xs">&copy; {new Date().getFullYear()} Spike AI. The world&apos;s first streaming platform for AI-generated cinema.</p></div></footer>
