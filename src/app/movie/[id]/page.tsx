...
export default function MoviePage() {
  ...
  if (loading) return (<div className="min-h-screen bg-[#050505] flex items-center justify-center"><div className="flex flex-col items-center gap-4"><div className="w-10 h-10 border-2 border-white border-t-transparent rounded-full animate-spin" /><p className="text-white/30 text-sm tracking-wider">Loading film...</p></div></div>);
      ...
      <section className="relative w-full aspect-video max-h-[75vh] overflow-hidden bg-black">
        {playing && embedUrl ? (
          ...
          <div className="relative w-full h-full cursor-pointer group" onClick={() => movie.video_url && setPlaying(true)}>
            ...
            {movie.video_url && (<div className="absolute inset-0 flex items-center justify-center"><div className="w-20 h-20 rounded-full bg-white/90 flex items-center justify-center shadow-2xl shadow-black/30 group-hover:scale-110 transition-transform duration-300"><Play size={32} fill="white" className="text-white ml-1" /></div></div>)}
          </div>
        )}
      </section>
