...
const CATEGORIES = ["Trending", "AI Horror", "Sci-Fi Visions", "Award Winning", "AI Anime", "AI Documentary", "Action", "Fantasy", "Runway Masterpieces"];
...
const AI_MODELS = ["Runway Gen-4", "Runway Gen-3", "Midjourney", "Stable Diffusion XL", "Stable Video", "Sora", "Kling AI", "Pika Labs", "ElevenLabs", "Hailuo", "Luma Dream Machine", "Seedance", "Veo3", "Wan 2.6", "Other"];
    ...
    <div className="min-h-screen bg-[#08080a] text-white">
      ...
      <div className="max-w-[1400px] mx-auto px-4 md:px-8 py-8">
        ...
        <div className="flex items-center gap-2 mb-8">
          ...
          <button onClick={() => setActiveTab("creators")} className={"px-5 py-2.5 rounded-xl text-[13px] font-bold tracking-wide transition-all cursor-pointer " + (activeTab === "creators" ? "bg-white/[0.08] text-white border border-white/[0.1]" : "text-white/30 hover:text-white/50")}><Users size={14} className="inline mr-2" />Users ({creators.length})</button>
          ...
          <div className="space-y-4">
            {creatorsLoading ? <div className="flex items-center justify-center py-20"><Loader2 size={24} className="animate-spin text-white/20" /></div> : creators.length === 0 ? <div className="text-center py-20 text-white/20">No creators yet</div> : (
              <div className="bg-[#111114] border border-white/[0.06] rounded-2xl overflow-hidden">
                <div className="grid grid-cols-12 gap-4 px-5 py-3 text-[10px] font-bold tracking-[0.15em] uppercase text-white/20 border-b border-white/[0.04]"><div className="col-span-1"></div><div className="col-span-2">Name</div><div className="col-span-2">Email</div><div className="col-span-1 text-center">Films</div><div className="col-span-2">Type</div><div className="col-span-2">Role</div><div className="col-span-1">Joined</div><div className="col-span-1"></div></div>
                {creators.map(c => (
                  <div key={c.id} className="grid grid-cols-12 gap-4 px-5 py-4 items-center border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors">
                    ...
                    <div className="col-span-1"><p className="text-[11px] text-white/20">{new Date(c.created_at).toLocaleDateString("en-GB",{day:"2-digit",month:"short",year:"numeric"})}</p></div>
                    <div className="col-span-1"><a href={"/creator/" + c.id} target="_blank" rel="noopener noreferrer" className="text-[11px] text-purple-400/60 hover:text-purple-300 transition-colors">View Profile</a></div>
                  </div>
                ))}
              </div>
            )}
