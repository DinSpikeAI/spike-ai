...
export default function BecomeCreatorPage() {
  ...
  if (!user) return (
    <Shell>
      <div className="text-center" style={{ animation: "reveal 0.7s cubic-bezier(0.16,1,0.3,1)" }}>
        ...
        <h1 className="text-[44px] md:text-[52px] font-bold tracking-[-0.02em] leading-[1.08] text-white mb-5" style={{ textShadow: "0 0 80px rgba(139,92,246,0.12)" }}>Sign in{"\n"}to apply.</h1>
        <p className="text-[17px] text-white/25 mb-14">You need an account to become a creator.</p>
  ...
  if (alreadyPending || submitted) return (
    <Shell>
      <div className="text-center" style={{ animation: "reveal 0.7s cubic-bezier(0.16,1,0.3,1)" }}>
        ...
        <p className="text-[17px] text-white/25 mb-3">We&apos;ll review and get back to you soon.</p>
  ...
  return (
    <Shell>
      <div className="w-full max-w-[480px] text-center" style={{ animation: "reveal 0.7s cubic-bezier(0.16,1,0.3,1)" }}>

        {/* Logo Icon */}
        <div className="inline-block mb-8">
          <div className="relative">
            <div className="w-20 h-20 rounded-[20px] bg-gradient-to-br from-purple-500/25 to-indigo-500/15 border border-purple-400/15 flex items-center justify-center backdrop-blur-sm">
              <Sparkles size={32} className="text-purple-300/60" />
            </div>
            <div className="absolute -inset-4 rounded-[28px] bg-purple-500/[0.06] blur-2xl -z-10" />
          </div>
        </div>
        ...
        <p className="text-[17px] text-white/25 mb-12">Join the future of AI cinema as a Pioneer Creator.</p>
        ...
        <div className="max-w-[420px] mx-auto">
          <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] backdrop-blur-xl p-7 shadow-[0_0_60px_rgba(0,0,0,0.3)]">
            <div className="space-y-5 text-left">
              ...
              {/* Bio */}
              <div>
                <label className="block text-[11px] font-semibold tracking-[0.2em] text-white/30 uppercase mb-2.5 ml-1">About You *</label>
                <textarea value={bio} onChange={(e) => { setBio(e.target.value); setError(null); }} rows={3}
                  ...
                  className="w-full px-5 py-[15px] bg-white/[0.03] border border-white/[0.08] rounded-xl text-[15px] text-white placeholder-white/15 focus:outline-none focus:border-purple-500/40 focus:bg-white/[0.05] focus:shadow-[0_0_20px_rgba(139,92,246,0.08)] transition-all resize-none" />
              </div>

              {/* Sample Work */}
              <div>
                <label className="block text-[11px] font-semibold tracking-[0.2em] text-white/30 uppercase mb-2.5 ml-1">Link to Your Work *</label>
                <input value={sampleWorkUrl} onChange={(e) => { setSampleWorkUrl(e.target.value); setError(null); }}
                  placeholder="YouTube, Vimeo, or portfolio"
                  className="w-full px-5 py-[15px] bg-white/[0.03] border border-white/[0.08] rounded-xl text-[15px] text-white placeholder-white/15 focus:outline-none focus:border-purple-500/40 focus:bg-white/[0.05] focus:shadow-[0_0_20px_rgba(139,92,246,0.08)] transition-all" />
              </div>
              ...
              {/* Website */}
              <div>
                <label className="block text-[11px] font-semibold tracking-[0.2em] text-white/30 uppercase mb-2.5 ml-1">Website</label>
              ...
              <div className="grid grid-cols-3 gap-2">
                <div>
                  ...
                  <input value={socialX} onChange={(e) => setSocialX(e.target.value)} placeholder="@handle"
                    className="w-full px-3 py-[13px] bg-white/[0.03] border border-white/[0.08] rounded-xl text-[13px] text-white placeholder-white/15 focus:outline-none focus:border-purple-500/40 transition-all" />
                </div>
                <div>
          ...
          <p className="text-[11px] text-white/10 text-center mt-6">
